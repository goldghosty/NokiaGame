...
func _on_area_entered(area: Area2D) -> void:
	if area.is_in_group("Projectile") and !mirror_broken and is_on_screen and Global.level_one_complete:
...
func _on_body_entered(body: Node2D) -> void:
	if body.is_in_group("Player") and Global.level_one_complete and mirror_broken == true:
